...
module Library
  ...
  attach_function :release_my_struct, :Release_Person, [:pointer], :void
end
```
...
If you'd like, put some `Console::WriteLine` statements in the `~_Person` destructor. When you call the `Library.get_information` from `irb`, when ruby garbage collects 
