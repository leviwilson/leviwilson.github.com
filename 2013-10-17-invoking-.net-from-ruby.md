...
but you will basically need to set the preprocessor variable `_VARIADIC_MAX=10` for it to compile. Once you've gotten that out of the way, begin by creating a new Win32 
Console application project.  Copy the `include`, `gtestd.lib` and `tgest.lib` (Debug and Release) somewhere into your project. I usually put it in a `vendor` folder. Here is what your `main` should look like:
